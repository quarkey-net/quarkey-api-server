...
class Register(object):
    ...
    def on_post(self, req, resp):
        ...
        if api_validate_form(req.media, self.post_form):
            ...
            with AppState.Database.CONN.cursor() as cur:
                cur.execute("SELECT id, email, roles FROM accounts WHERE email = %s", (req.media["email"],))
                q2 = cur.fetchone()
            ...
            if q2 is None:
                ...
                with AppState.Database.CONN.cursor() as cur:
                    try:
                        cur.execute(
                            "INSERT INTO accounts (id, username, email, password) VALUES (%s, %s, %s, %s)",
                            (
                                uid.hex,
                                req.media['username'],
                                req.media['email'],
                        ...
                        cur.execute("UPDATE tester_keys SET f_owner = %s WHERE id = %s", (uid.hex, key_id))
                ...
                # response success
                api_message('d', "successfull register (username=\"{0}\", uid={1})".format(req.media["username"], uid.hex))
